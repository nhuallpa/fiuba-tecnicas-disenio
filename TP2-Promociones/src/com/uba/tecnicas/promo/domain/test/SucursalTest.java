...
public class SucursalTest {
	...
	public void setUp() throws Exception {
		...
		coca = new Producto("CocaCola", 1, "Bebidas");
		sprite = new Producto("Sprite", 1, "Bebidas");
		maceta = new Producto("Maceta", 10, "Jardineria");
		...
		Collection<Oferta> ofertas = new Vector<Oferta>();
		ofertas.add(oferta);
		caja1 = new Caja(ofertas, sucursal);
		caja2 = new Caja(ofertas, sucursal);
	}
	...
	@Test
	public void testCocaMasSpite30Descuento() throws MarketException {
		caja1.abrir(Calendar.getInstance());		
		caja1.iniciarVenta();
		caja1.agregarProducto(coca, 3);
		caja1.agregarProducto(sprite, 2);
		caja1.agregarProducto(maceta, 1);
		caja1.finalizarVenta(FormaPago.CREDITO);
		assertEquals(14.4, caja1.getVenta().getTotal(), 0.05);
	}
	
	@Test
	public void testOtraVentaCocaMasSpite30Descuento() throws MarketException {
		caja1.abrir(Calendar.getInstance());		
		caja1.iniciarVenta();
		caja1.agregarProducto(coca, 1);
		caja1.agregarProducto(sprite, 4);
		caja1.agregarProducto(maceta, 4);
		caja1.finalizarVenta(FormaPago.CREDITO);
		assertEquals(44.7, caja1.getVenta().getTotal(), 0.05);
	}
}
